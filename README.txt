...
│  
├── src/                    # Source code
│   ├── scrape.ipy          # Step 1: Data Collection
│   ├── preprocess.ipy      # Step 2: Cleaning & Splitting
│   ├── train.ipy           # Step 3: Fine-tuning
│   └── evaluate.ipy        # Step 4: Evaluation & Inference
...
2.  **Data Collection:**
    ...
    ```bash
    jupyterlab src/scrape.ipy
    ```

3.  **Preprocessing:**
    Cleans the data and creates Training (90%) and Validation (10%) splits in JSON format.
    ```bash
    jupyterlab src/preprocess.ipy
    ```

4.  **Training:**
    Fine-tunes GPT-2 on the processed data. Logs loss and eval metrics to the console.
    ```bash
    jupyterlab src/train.ipy
    ```

5.  **Evaluation:**
    Calculates Perplexity and generates sample text based on BMW-themed prompts.
    ```bash
    jupyterlab src/evaluate.ipy
    ```


### 1. Data Collection
I used `BeautifulSoup` over heavier tools like Selenium because the BMW press site is relatively static for text content. This keeps the pipeline lightweight. I chose to concatenate the "Title" and "Text" of the articles to ensure the model learns the relationship between headlines and content.
